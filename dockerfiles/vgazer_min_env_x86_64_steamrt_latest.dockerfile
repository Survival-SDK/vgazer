...
RUN apt-get update \
    ...
    && update-alternatives --install /usr/bin/gcc-ar gcc-ar \
     /usr/bin/gcc-ar-9 1 \
    && update-alternatives --install /usr/bin/gcc-nm gcc-nm \
     /usr/bin/gcc-nm-9 1 \
...
RUN ./configure --prefix=/usr/local --disable-test-modules --with-lto=yes \
    ...
    && update-alternatives --install /usr/bin/pip3 \
     pip3 /usr/local/bin/pip3.11 1
WORKDIR /tmp
