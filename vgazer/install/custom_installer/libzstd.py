...
def Install(auth, software, platform, platformData, mirrors, verbose):
    ...
    releases = auth["github"].GetJson(
     "https://api.github.com/repos/facebook/zstd/releases")
    ...
    try:
        ...
        with WorkingDir(extractedDir + "/build/cmake"):
            RunCommand(["mkdir", "builddir"], verbose)
        buildDir = os.path.join(extractedDir, "build", "cmake", "builddir")
        with WorkingDir(buildDir):
            RunCommand(
             ["cmake", "..",
              "-DCMAKE_TOOLCHAIN_FILE=" + configCmake.GetCrossFileName(),
              "-DCMAKE_INSTALL_PREFIX=" + installPrefix,
              "-DZSTD_BUILD_PROGRAMS=OFF", "-DCMAKE_VERBOSE_MAKEFILE:BOOL=ON"],
             verbose)
