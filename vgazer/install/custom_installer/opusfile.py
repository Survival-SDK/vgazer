...
def GetTarballUrl():
    ...
    for link in links:
        if "opusfile " in link.text:
            version = link.text.split(" ")[1]
            return ("https://archive.mozilla.org/pub/opus/opusfile-" + version
             + ".tar.gz")

    raise TarballLost(
     "Unable to find tarball with last stable release of opusfile")
