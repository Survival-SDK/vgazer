...
class MirrorsBase():
    ...
    def __init__(self, siteName, getMirrorsList):
        print("Checking mirrors list for " + siteName + "...")
        ...
        if self.storeHome.ResolveDirectory():
            if self.ResolveHomeDirsAndFiles(self.storeHome, siteName):
                data = self.storeHome.SubdirectoryReadTextFile(siteName,
                 "mirrors")
                if data == "":
                    print("Stored mirrors list for " + siteName + " not found")
                    ...
                    print(
                     "Stored mirrors list for " + siteName + " out of date")
                    ...
                    print("Found up to date mirrors list for " + siteName)
                    self.mirrorsList = self.MirrorsListFromText(data)
            else:
                print("Unable to load stored mirrors list for " + siteName)
                self.mirrorsList = getMirrorsList()
        else:
            print("Unable to load stored mirrors list for " + siteName)
            self.mirrorsList = getMirrorsList()
